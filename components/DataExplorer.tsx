    ...
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden animate-fade-in flex flex-col min-h-[600px]">
      ...
      <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        <div className="flex items-center text-xs space-x-2 text-slate-500 font-medium">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="text-slate-900">Profil-Pengguna</span>
        </div>
        <div className="flex space-x-2">
          <Button variant="outline" className="h-8 text-xs py-0">Semak Audit</Button>
          <Button className="h-8 text-xs py-0 bg-brand-600 hover:bg-brand-700 border-none text-white">Visual data</Button>
        </div>
      </div>
      ...
      <div className="p-4 bg-white border-b border-slate-100 flex items-center space-x-3">
        ...
        <Button onClick={handleFind} className="bg-brand-600 hover:bg-brand-700 h-9 px-4 text-xs">Cari</Button>
      ...
      <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
        <div className="flex space-x-2">
          <button className="flex items-center space-x-1.5 px-3 py-1.5 bg-brand-600 text-white rounded text-xs font-bold hover:bg-brand-700 transition-colors">
      ...
      <div className="flex-1 overflow-auto p-6 bg-white">
        {isLoading ? (
          <div className="h-full flex flex-col items-center justify-center space-y-3 opacity-50">
             ...
             <span className="text-sm font-medium">Memuatkan dokumen...</span>
          </div>
        ) : documents.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-20">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-2 max-w-sm">
                Sila tambah atau import data JSON untuk paparan pangkalan data.
              </p>
            </div>
            <Button className="bg-brand-600 hover:bg-brand-700 border-none text-white">Import Data</Button>
          </div>
        ) : (
          <div className="space-y-4">
            {documents.map((doc, i) => (
              <div key={i} className="group border border-slate-100 rounded-lg hover:border-brand-200 transition-colors overflow-hidden">
      ...
      <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center text-[10px] text-slate-400 space-x-4">
         <div className="flex items-center space-x-1">
           ...
           <span>Gunakan butang "Cari" untuk melaksanakan tapisan dokumen anda.</span>
         </div>
