...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Memulakan sambungan sistem pangkalan data selamat
    const initDb = async () => {
  ...
  const handleLogin = async (name: string, email: string) => {
    // Semak kecocokan profil pengguna berdaftar
    let existingUser = await dbService.findUser(email);
    
    if (!existingUser) {
      // Daftarkan pengguna baru di pangkalan data pada cubaan log masuk kali pertama
      ...
      <div className="hidden lg:flex w-1/2 relative bg-brand-900 overflow-hidden text-white flex-col justify-between p-12">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <div className="flex items-center space-x-2 text-brand-300 bg-white/5 border border-white/10 rounded-full px-4 py-2 w-fit">
            ...
            <span className="text-sm font-medium">Sistem Kawalan Zero Trust</span>
          </div>
        </div>
